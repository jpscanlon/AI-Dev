...
namespace AIDevServer
{
    static class Knowledgebase
    {
        ...
        private static List<Net> Nets = new List<Net>();
        ...
        private static float targetError = 0.1F;  // Training is complete when it reaches this error value.
        //private static bool useGrayScale = true;
        private static bool useTemperature = false;  // If false, only changes that decrease error will be accepted.
        ...
        private static int numEpochsPerTempChange = 50;  // Usually 100-1,000 error compares per temp change.
        ...
        private static bool useSignificantOutputFactor = false;
        ...
        private static bool writeOutputsToLog = false;
            ...
            try
            {
                ...
                using (SqlDataReader reader = sql.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        netsData = netsData + string.Format("{0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}",
        ...
        public static string CreateNet()
        {
            ...
            int numInputs = 2;   // Simplest useful number of inputs.
            int numOutputs = 1;  // Simplest output, binary.
            int numFCLayers = 1;  // 1 fully connected layer.
            int numConvLayers = 0;  // 0 convolutional layers.

            // Temporary default properties.
            numInputs = 784;  // Size of an EMINST image input map.
            numOutputs = 10;  // 10 digits.
            numFCLayers = 2;  // 2 fully connected layers.
            numConvLayers = 0;  // 0 convolutional layers.
        ...
        public static string ReinitializeNet()
        {
            ...
            response = "Reinitialized weights.";
        ...
        {
            string response = "commanderror";
            int i = 0;
            string oldName = "";
            string newName = "";
            string type = "";
            string activationFunction = "";
            string isGrayscale = "True";

            args = args.Trim();
            oldName = ReadNextArg(args, ref i);
            newName = ReadNextArg(args, ref i);
            type = ReadNextArg(args, ref i);
            activationFunction = ReadNextArg(args, ref i);
            isGrayscale = ReadNextArg(args, ref i);
        ...
        public static string ModifyAndSaveNet(string args)
        {
            ...
            activationFunction = ReadNextArg(args, ref i);
            isGrayscale = ReadNextArg(args, ref i);
            numInputs = ReadNextArg(args, ref i);
            numOutputs = ReadNextArg(args, ref i);
            numFCLayers = ReadNextArg(args, ref i);
            numConvLayers = ReadNextArg(args, ref i);
        ...
        public static string RunNet(string args)
        {
            ...
            //Bitmap inputImage = new Bitmap(@"C:\Users\Admin\Data\AI Dev Visual Inputs\Datasets\EMINST\" +
        ...
        {
            string trainingDataPath = @"C:\Users\Admin\Data\AI Dev Visual Inputs\Datasets\EMINST - Reduced Sets";

            switch (trainingDataset)
            {
                case TrainingDataset.dataset1Item:
                    trainingDataPath = trainingDataPath + @"\mnist_png - (1 & 1)\training";
                    //iterationSize = 10;
                    break;
                case TrainingDataset.dataset2Item:
                    trainingDataPath = trainingDataPath + @"\mnist_png - (1 & 2)\training";
                    //iterationSize = 20;
                    break;
                case TrainingDataset.dataset00_2Percent:
                    trainingDataPath = trainingDataPath + @"\mnist_png - 0.1 & 0.2 Percent\training";
                    //iterationSize = 60;
                    break;
                case TrainingDataset.dataset02_0Percent:
                    trainingDataPath = trainingDataPath + @"\mnist_png - 1 & 2 Percent\training";
                    //iterationSize = 600;
                    //iterationSize = 100;
                    break;
                case TrainingDataset.dataset10_0Percent:
                    trainingDataPath = trainingDataPath + @"\mnist_png - 10 Percent\training";
                    //iterationSize = 600;
                    break;
            }
        ...
        private static void SetTrainingParameters(string args)
        {
            int i = 0;
            string dataset = "";
            string iterationSize = "";

            args = args.Trim();
            dataset = ReadNextArg(args, ref i);
            iterationSize = ReadNextArg(args, ref i);
        ...
        private static void ReturnErrorHistory(List<float> errorHistory)
        {
            ...
            else
            {
                ...
                for (int epoch = 1; epoch <= errorHistory.Count; epoch = epoch + 10)
        ...
        {
            float[] netTargetOutputs = new float[Nets[currentNet].NumOutputs];
            float[] netOutputs = new float[Nets[currentNet].NumOutputs];
                ...
                image < trainingImages.Count; image++)
            {
                ...
                sumOfIterationErrors = sumOfIterationErrors + error;
                imagesTested++;
        ...
        private static float GetError(float[] netTargetOutputs, float[] netOutputs, int significantOutput = -1)
        {
            ...
            for (int output = 0; output < netTargetOutputs.Length; output++)
            {
                ...
                error = error + Math.Pow(netTargetOutputs[output] - netOutputs[output], 2);
        ...
        public static string LoadNet(string name)
        {
            ...
            try
            {
                ...
                net = (Net)formatter.Deserialize(stream);
                stream.Close();
